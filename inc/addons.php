 ...
 */
function maxwell_amp_menu_dropdown_toggles( $item_output, $item, $depth, $args ) {
	...
	*/
	...
	// Add SVG icon.
	$dropdown_button .= $expanded ? maxwell_get_svg( 'collapse' ) : maxwell_get_svg( 'expand' );
	...
	$dropdown_button .= sprintf(
		...
		esc_attr( sprintf( "$expanded_state_id ? %s : %s", wp_json_encode( esc_html__( 'Collapse child menu', 'maxwell' ) ), wp_json_encode( esc_html__( 'Expand child menu', 'maxwell' ) ) ) ),
		esc_html( $expanded ? esc_html__( 'Collapse child menu', 'maxwell' ) : esc_html__( 'Expand child menu', 'maxwell' ) )
	);
...
add_filter( 'walker_nav_menu_start_el', 'maxwell_amp_menu_dropdown_toggles', 10, 4 );
