...
function maxwell_gutenberg_support() {
	...
	add_theme_support( 'editor-color-palette', apply_filters( 'maxwell_editor_color_palette_args', array(
		array(
...
function maxwell_block_editor_assets() {
	...
	wp_enqueue_style( 'maxwell-editor-styles', get_theme_file_uri( '/assets/css/gutenberg-styles.css' ), array(), '20210306', 'all' );

	// Enqueue Page Template Switcher Editor plugin.
	#wp_enqueue_script( 'maxwell-page-template-switcher', get_theme_file_uri( '/assets/js/page-template-switcher.js' ), array( 'wp-blocks', 'wp-element', 'wp-edit-post' ), '20210306' );
}
 ...
 */
function maxwell_block_editor_settings( $editor_settings ) {
...
#add_filter( 'block_editor_settings', 'maxwell_block_editor_settings', 11 );


/**
 * Add body classes in Gutenberg Editor.
 */
function maxwell_block_editor_body_classes( $classes ) {
	global $post;
	...
	if ( 'templates/template-fullwidth.php' === get_page_template_slug( $post->ID ) ) {
		$classes .= ' maxwell-fullwidth-page-layout ';
	}
	...
	if ( 'templates/template-no-title.php' === get_page_template_slug( $post->ID ) or
		...
		$classes .= ' maxwell-page-title-hidden ';
	}

	// Full-width / No Title Page Template?
	if ( 'templates/template-fullwidth-no-title.php' === get_page_template_slug( $post->ID ) ) {
		$classes .= ' maxwell-fullwidth-page-layout maxwell-page-title-hidden ';
	}

	return $classes;
}
#add_filter( 'admin_body_class', 'maxwell_block_editor_body_classes' );
